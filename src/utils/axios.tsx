...
export function authenticatedInstance({headersProps, params}: AuthenticatedParamType) {
    const headers = {
        ...headersProps,
    }
...
export function callPostAPI({ route, body, isAuthenticated = true, headersProps = {}, params}: PostParamsType) {
    if (isAuthenticated) {
...
export function callGetAPI({ route, isAuthenticated = true, params}: GetParamsType) {
    if (isAuthenticated) {
